...
#
# Each photo produces two files: a full version (long edge 1600px, q65) used by
# the lightbox, and a light thumbnail (900px, q60) in <slug>/thumbs/ used by the
# gallery grid. Both are renamed sequentially; a fresh assets/data.js is written.
...
for entry in "${MAP[@]}"; do
  ...
  mkdir -p "$OUT/$slug/thumbs"
  rm -f "$OUT/$slug"/*.jpg "$OUT/$slug/thumbs"/*.jpg 2>/dev/null || true
  ...
  while IFS= read -r f; do
    ...
    thumb="$OUT/$slug/thumbs/${prefix}-${num}.jpg"
    sips -s format jpeg -s formatOptions 65 --resampleHeightWidthMax 1600 "$f" --out "$target" >/dev/null 2>&1
    if [ -f "$target" ]; then
      sips -s format jpeg -s formatOptions 60 --resampleHeightWidthMax 900 "$target" --out "$thumb" >/dev/null 2>&1
      ...
      printf '    {"src":"assets/img/%s/%s-%s.jpg","thumb":"assets/img/%s/thumbs/%s-%s.jpg","w":%s,"h":%s}' "$slug" "$prefix" "$num" "$slug" "$prefix" "$num" "$w" "$h" >> "$MANIFEST"
    fi
